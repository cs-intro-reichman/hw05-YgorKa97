...
public class GameOfLife {
	...
	private static void test2(String fileName) {
		...
		System.out.println("count alive: " + cellValue(board, 3, 3));
		System.out.println("count neighbors: " + count(board, 1, 1));
	}
	...
	public static int[][] read(String fileName) {
		...
		boolean stopReading = false;// boolean value to know when to stop.
		int srartRow = 1;// our row starts from 1.
		String nextLine = "";// empty string for import file str.
		while (!stopReading) {// while loop to tell program to start read file.
			nextLine = in.readLine();
			for (int i = 0; i < nextLine.length(); i++) {
				if (nextLine.charAt(i) == 'x') {
					board[srartRow][i + 1] = 1;// print pattern from point (1,1).
				}
			}
			stopReading = in.isEmpty();// if its end of a line stop reading the line.
			srartRow++;// addinf 1 to row.
		}
	...
	public static int[][] evolve(int[][] board) {
		//// Replace the following statement with your code.
		int[][] newBoard = new int[board.length][board[0].length];// creates new board.
		for (int i = 1; i < board.length - 1; i++) {
			for (int j = 1; j < board[0].length - 1; j++) {// new board starts from point(1,1).
				newBoard[i][j] = cellValue(board, i, j);// new board contains cellValue function.
			}
		}

		return newBoard; // returns newBoard with cellValue in it.
	}
	...
	public static int cellValue(int[][] board, int i, int j) {
		//// Replace the following statement with your code.
		int cell = board[i][j]; // stores cellValue board.
		int neighbors = count(board, i, j); // stores count of neighbors.
		if (cell == 1) { // if cell is alive
			if (neighbors == 2 || neighbors == 3) { // If the cell is alive and has two or three live neighbors, it
													// remains alive.
				return 1;
			} else { // If the cell is alive and has more than three live neighbors, it dies.
				return 0;
			}
		} else {
			if (neighbors == 3) { // If the cell is dead and and has three live neighbors, it becomes alive.
				return 1;
			}
		}

		return 0; // returns 0 if no naibors and no cells alive.
	}
	...
	public static void print(int[][] arr) {
		...
		for (int i = 1; i < (arr.length - 1); i++) {
			for (int j = 1; j < (arr[0].length - 1); j++) { // loop that buinds empty 2D array.
				System.out.printf("%3s", arr[i][j]); // printing board with spases.
			}
			System.out.println();
		}
